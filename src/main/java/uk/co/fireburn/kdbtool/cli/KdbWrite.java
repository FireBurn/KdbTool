...
final class KdbWrite {
    ...
    static int convert(Args a) throws Exception {
        ...
        for (KdbRecord r : db.records()) {
            ...
            if (r.hasPrivateKey()) {
                // Reassemble leaf..root from the signer records rather than relying on the
                // JDK PKCS#12 store to relink the separate trusted entries.
                p12.setKeyEntry(r.label(), r.privateKey(pw), tpw,
                    KdbCommands.assembleChain(db, r.certificate())
